    ...
    def get_search_result(self):
        ...
        for prd in prd_list:
            ...
            result_dict = {
                ...
                'product_price': self.text_clean.run_clean( "__spchar__",prd_li_iter["price"].get("couponDiscountedBinPrice","-")) ,
...
# 네이버쇼핑 검색결과
class MallNaverShoppingSearch(Mall) : 
    # 자식클래스 추가변수 : 키워드
    kwd = "검색대상 키워드"
    kwd_encoded = requests.utils.quote(kwd)
    mall_name = "naverShopping"
    url_main = "https://www.naver.com/"
    search_result_url = "https://search.shopping.naver.com/search/all?query="

    # 검색결과 화면 - 셀렉터
    # 네이버쇼핑은 nextJS로 만들어져 정적 페이지 데이터 JSON을 가져와야 함
    selector_search_result = {
    ...
    # 네이버쇼핑은 json을 가져와 직접 넣기 때문에 필요없음
    # 검색결과에서 상품목록 뽑기
    def get_search_result(self):
        print(self.kwd)
        kwd_encoded = requests.utils.quote(self.kwd)
        ...
        # 상품 리스트 수집 시작
        next_json_str = str(soup.select("#__NEXT_DATA__")[0].contents[0]) 

        # next_json_str = self.get_content_from_selector(soup,"#__NEXT_DATA__","","" )  
        prd_list = json.loads(next_json_str)["props"]["pageProps"]["initialState"]["products"]["list"]
        print("상품 갯수 :" + str(prd_list.__len__()))

        # 상품목록 저장
        for prd_li in prd_list:
            prd_li_iter = prd_li["item"]
            productId = prd_li_iter["id"]
            result_dict = {
                "product_title": prd_li_iter.get("productTitle","-"),
                "product_thumbnail":prd_li_iter.get("imageUrl","-"),
                'product_price': prd_li_iter.get("lowPrice","-"),
                'product_rating_cnt': prd_li_iter.get("reviewCount","-"),
                'product_rating': prd_li_iter.get("scoreInfo","-"),
                'product_url': prd_li_iter.get("adcrUrl", "https://search.shopping.naver.com/catalog/" + productId),
            }
            productListResult.append(result_dict)
        return productListResult
